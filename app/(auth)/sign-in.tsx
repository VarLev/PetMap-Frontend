...
const SignIn = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      const creds = await userStore.singInUser(email, password);
      if (creds) {
        logLogin('email');
        const user = userStore.currentUser;
        if (user?.userStatus === UserStatus.Onboarding) router.replace('(auth)/onboarding');
        else router.replace('/screenholder');
      }
    } catch (error: any) {
        ...
        <View className="w-full justify-between h-full px-9 my-10">
          <View>
            ...
            <View className="justify-start mt-10 ">
              <View className="flex-col items-start justify-center">
                <Text variant="titleSmall" className="text-xl mb-2 font-nunitoSansBold">
                  {i18n.t('signIn.welcomeBack')}
                </Text>
                <Text variant="titleSmall" className="pb-4 text-sm font-nunitoSansRegular">
          ...
          <View className="pb-20">
            <View className="justify-center flex-row gap-2">
              <Text className="text-base text-gray-500 font-nunitoSansRegular">{i18n.t('signIn.noAccount')}</Text>
              <Link href="/sign-up" className="text-base text-indigo-800 font-nunitoSansBold">
