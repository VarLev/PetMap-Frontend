  ...
  return (
    <View className="relative items-center justify-center">
      ...
      <View className="absolute bottom-2 flex-row space-x-2">
        <TouchableOpacity activeOpacity={0.8} className="bg-purple-100 w-10 h-10 rounded-full items-center justify-center" onPress={onReplace}>
          <MaterialIcons name="loop" size={25} color="black" />
        </TouchableOpacity>
        {!onChooseAvatar && (<TouchableOpacity activeOpacity={0.8} className="bg-red-500 w-10 h-10 rounded-full items-center justify-center" onPress={onDelete}>
          <MaterialIcons name="delete-outline" size={25} color="white" />
        </TouchableOpacity>)}
        {onChooseAvatar && (
          <TouchableOpacity activeOpacity={0.8} className="bg-indigo-800 w-10 h-10 rounded-full items-center justify-center" onPress={onChooseAvatar}>
